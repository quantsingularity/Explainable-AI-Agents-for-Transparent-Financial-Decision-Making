...
class HumanStudySimulator:
    ...
    def simulate_study(self, explanations: List[Dict[str, Any]], 
                      n_participants: int = 120) -> pd.DataFrame:
        """
        ...
        for condition in conditions:
            for participant_id in range(participants_per_condition):
                ...
                trust_score = np.clip(
                    base_trust + np.random.normal(0, trust_noise * 0.3),
                    1.0, 5.0
                )
                
                decision_accuracy = np.clip(
                    base_accuracy + np.random.normal(0, accuracy_noise),
                    0.0, 1.0
                )
                ...
                completion_time = base_time + time_adjustment.get(condition, 0) + \
                ...
                results.append({
                    ...
                    'explanation_helpful': 1 if condition != 'no_explanation' and trust_score > 3.5 else 0
                })
        ...
        for condition in conditions:
            ...
            logger.info(f"\n{condition.upper()}:")
            logger.info(f"  Trust: {cond_data['trust_score'].mean():.2f} ± {cond_data['trust_score'].std():.2f}")
            logger.info(f"  Accuracy: {cond_data['decision_accuracy'].mean():.2%} ± {cond_data['decision_accuracy'].std():.2%}")
    ...
    def compute_effect_sizes(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Cohen's d for trust
        trust_d = (narrative['trust_score'].mean() - no_exp['trust_score'].mean()) / \
                  np.sqrt((narrative['trust_score'].var() + no_exp['trust_score'].var()) / 2)
        
        # Cohen's d for accuracy
        acc_d = (narrative['decision_accuracy'].mean() - no_exp['decision_accuracy'].mean()) / \
                np.sqrt((narrative['decision_accuracy'].var() + no_exp['decision_accuracy'].var()) / 2)
        
        # Statistical tests
        trust_t, trust_p = stats.ttest_ind(narrative['trust_score'], no_exp['trust_score'])
        acc_t, acc_p = stats.ttest_ind(narrative['decision_accuracy'], no_exp['decision_accuracy'])
    ...
    def generate_qualitative_feedback(self, n_samples: int = 10) -> List[str]:
        ...
        return list(np.random.choice(feedback_templates, min(n_samples, len(feedback_templates)), replace=False))
...
class CounterfactualGenerator:
    ...
    def generate_counterfactual(self, model: Any, X: np.ndarray,
        ...
        if hasattr(model, 'predict_proba'):
            current_prob = model.predict_proba(X_counter.reshape(1, -1))[0, desired_class]
        else:
        ...
        for iteration in range(max_iterations):
            ...
            for feature_idx in range(len(X_counter)):
                ...
                if hasattr(model, 'predict_proba'):
                    prob_plus = model.predict_proba(X_temp.reshape(1, -1))[0, desired_class]
                else:
                ...
                if hasattr(model, 'predict_proba'):
                    prob_minus = model.predict_proba(X_temp.reshape(1, -1))[0, desired_class]
                else:
        ...
        return {
            ...
            'changes': changes,
            'achieved_desired_class': pred == desired_class if hasattr(model, 'predict') else False,
