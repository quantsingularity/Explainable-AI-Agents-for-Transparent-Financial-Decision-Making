...
class XAIAgent:
    ...
    def initialize(self, model: Any, X_train: np.ndarray, feature_names: list):
        ...
        if self.method == "shap":
            ...
            background = shap.sample(X_train, min(100, len(X_train)), random_state=self.seed)
    ...
    def explain(self, X: np.ndarray, idx: int) -> Dict[str, Any]:
        ...
        if self.method == "shap":
            ...
            attributions = dict(zip(
                self.feature_names, 
                attributions_tensor.detach().numpy()[0]
            ))
    ...
    def get_faithfulness_score(self, model: Any, X: np.ndarray, 
                               idx: int, top_k: int = 5) -> float:
        """
