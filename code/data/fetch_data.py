...
class DataFetcher:
    ...
    def _generate_synthetic_lending_data(self, mode: str) -> pd.DataFrame:
        ...
        loan_amount = (annual_income * np.random.uniform(0.2, 0.5, n_samples)).clip(
            1000, 50000
        )
        ...
        loan_status = (
            approval_prob + np.random.normal(0, 0.1, n_samples) > 0.5
        ).astype(int)
        ...
        logger.info(
            f"Synthetic data generated: {len(df)} samples, "
            f"{df['loan_status'].mean():.1%} approval rate"
        )

        return df

    def _process_lending_data(
        ...
        df_encoded = pd.get_dummies(
            df, columns=["home_ownership", "loan_purpose"], drop_first=True
        )
        ...
        logger.info(
            f"Processed data: {len(df_encoded)} samples, {len(feature_cols)} features"
        )

        return df_encoded, feature_cols

    def prepare_train_test_split(self, df: pd.DataFrame, feature_cols: list) -> Tuple:
        """
