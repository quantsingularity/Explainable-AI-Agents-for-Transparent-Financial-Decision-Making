...
ax.set_title('Explanation Generation Time by Method and Model', fontweight='bold', fontsize=14)
ax.set_xticks(x_pos)
...
plt.tight_layout()
plt.savefig(os.path.join(figures_dir, 'explanation_time_analysis.png'), 
            bbox_inches='tight', dpi=300)
plt.close()
...
sns.heatmap(heatmap_data, annot=True, fmt='.3f', cmap='YlGnBu', 
            ax=ax, cbar_kws={'label': 'Score'}, vmin=0.6, vmax=0.9)
...
plt.tight_layout()
plt.savefig(os.path.join(figures_dir, 'comprehensive_metrics_heatmap.png'), 
            bbox_inches='tight', dpi=300)
plt.close()
