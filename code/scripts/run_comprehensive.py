...
def run_comprehensive_experiment(mode: str = "full", seed: int = 42):
    ...
    for model_type, xai_method in model_configs:
        ...
        for idx in explain_indices:
            try:
                ...
                explanation_times.append(
                    explanation["metadata"]["elapsed_time_seconds"]
                )
        ...
        logger.info(
            f"  XAI Metrics: Faithfulness={metrics['mean_faithfulness']:.3f}, "
            f"Fidelity={metrics['mean_fidelity']:.3f}, "
            f"Completeness={metrics['mean_completeness']:.3f}"
        )
        logger.info(
            f"  Time: {exp_time:.1f}s total, {metrics['mean_explanation_time']:.3f}s per explanation"
        )
    ...
    if len(shap_results) > 0 and len(lime_results) > 0:
        ...
        stats_results = {"shap_vs_lime_faithfulness": faith_test}
    ...
    logger.info(
        f"  Accuracy: Cohen's d = {effect_sizes['accuracy_cohens_d']:.3f}, p = {effect_sizes['accuracy_p_value']:.4f}"
    )
    ...
    summary = {
        ...
        "best_xai_method": results_df.loc[
            results_df["mean_faithfulness"].idxmax(), "xai_method"
        ],
    ...
    logger.info("=" * 80)
    logger.info(
        f"Total Runtime: {total_time/60:.1f} minutes ({total_time/3600:.2f} hours)"
    )
    ...
    logger.info(
        f"Best Faithfulness: {summary['best_faithfulness']:.3f} ({summary['best_xai_method']})"
    )
