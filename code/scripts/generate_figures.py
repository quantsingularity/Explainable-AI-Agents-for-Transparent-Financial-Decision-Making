...
def generate_architecture_diagram(output_dir: str):
    ...
    ax.set_title(
        "Multi-Agent XAI System Architecture", fontsize=14, weight="bold", pad=20
    )

    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, "system_architecture.png"),
        bbox_inches="tight",
        dpi=300,
    )
    plt.close()
...
def generate_sequence_diagram(output_dir: str):
    ...
    for from_idx, to_idx, label, dy in messages:
        ...
        if from_idx == to_idx:
            ...
            ax.annotate(
                "", xy=(x2, y), xytext=(x1, y), arrowprops=dict(arrowstyle="->", lw=1.2)
            )
    ...
    ax.set_title(
        "Orchestration Sequence for Generating Explanation",
        fontsize=13,
        weight="bold",
        pad=20,
    )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, "orchestration_sequence.png"),
        bbox_inches="tight",
        dpi=300,
    )
    plt.close()
...
def generate_tradeoff_plot(results_df: pd.DataFrame, output_dir: str):
    ...
    ax.set_title(
        "Performance vs Explainability Tradeoff", fontsize=14, weight="bold", pad=15
    )
    ...
    # Add annotations
    ax.axhline(
        y=0.7, color="red", linestyle="--", alpha=0.5, label="Target Performance"
    )
    ax.axvline(
        x=0.75, color="green", linestyle="--", alpha=0.5, label="Target Faithfulness"
    )

    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, "perf_vs_explainability.png"),
        bbox_inches="tight",
        dpi=300,
    )
    plt.close()
...
def generate_xai_comparison(results_df: pd.DataFrame, output_dir: str):
    ...
    for xai_method in xai_methods:
        ...
        method_data["std_faithfulness"].values

        ax.bar(
            [i + 0.25 * list(xai_methods).index(xai_method) for i in x],
            y,
            width=0.25,
            label=xai_method.upper(),
            alpha=0.8,
        )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, "xai_comparison.png"), bbox_inches="tight", dpi=300
    )
    plt.close()
...
def generate_human_trust_plot(output_dir: str):
    ...
    ax.set_title(
        "User Trust by Explanation Type\n(n=120, synthetic data)",
        fontsize=12,
        weight="bold",
    )
    ax.set_xticks(x)
    ...
    ax.set_title(
        "User Decision Quality by Explanation Type\n(n=120, synthetic data)",
        fontsize=12,
        weight="bold",
    )
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, "human_trust_results.png"),
        bbox_inches="tight",
        dpi=300,
    )
    plt.close()
