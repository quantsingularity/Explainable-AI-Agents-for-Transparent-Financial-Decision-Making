...
for name, (x, y, color) in components.items():
    box = plt.Rectangle(
        (x - 0.8, y - 0.3), 1.6, 0.6, facecolor=color, edgecolor="black", linewidth=2
    )
    ax.add_patch(box)
...
plt.tight_layout()
plt.savefig(
    os.path.join(figures_dir, "system_architecture.png"), bbox_inches="tight", dpi=300
)
plt.close()
...
for name, x in zip(actors, actor_x):
    rect = plt.Rectangle(
        (x - 0.3, 19), 0.6, 0.8, facecolor="lightblue", edgecolor="black", linewidth=1.5
    )
    ax.add_patch(rect)
...
messages = [
    ...
    (2, 1, "Decision", 17.1),
    (1, 3, "Explain", 16.4),
    (3, 1, "Attribution", 15.7),
    (1, 4, "Narrate", 15.0),
    (4, 1, "Explanation", 14.3),
    (1, 0, "Response", 13.6),
]
...
plt.tight_layout()
plt.savefig(
    os.path.join(figures_dir, "orchestration_sequence.png"),
    bbox_inches="tight",
    dpi=300,
)
plt.close()
...
plt.tight_layout()
plt.savefig(
    os.path.join(figures_dir, "perf_vs_explainability.png"),
    bbox_inches="tight",
    dpi=300,
)
plt.close()
...
for i, xai in enumerate(xai_methods):
    ...
    ax.bar(
        x + i * width, data["mean_faithfulness"], width, label=xai.upper(), alpha=0.8
    )
...
for i, xai in enumerate(xai_methods):
    data = results_df[results_df["xai_method"] == xai]
    ax.bar(
        x + i * width, data["mean_completeness"], width, label=xai.upper(), alpha=0.8
    )
...
plt.tight_layout()
plt.savefig(
    os.path.join(figures_dir, "xai_comparison.png"), bbox_inches="tight", dpi=300
)
plt.close()
...
ax.set_title(
    "Decision Quality by Explanation\n(n=120, synthetic)", fontsize=12, weight="bold"
)
ax.set_xticks(x)
ax.set_xticklabels(conditions, rotation=15, ha="right")
ax.set_ylim(0, 1.0)
...
plt.tight_layout()
plt.savefig(
    os.path.join(figures_dir, "human_trust_results.png"), bbox_inches="tight", dpi=300
)
plt.close()
